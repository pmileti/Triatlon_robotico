...
#        return(math.sqrt(self.position[0]*self.position[0]+self.position[2]*self.position[2])>lossDistance)
    ...
    def outOfDohyo(self) ->bool:
        ...
        if(math.sqrt(self.position[0]*self.position[0]+self.position[2]*self.position[2])>(lossDistance-0.10)):
            penalizaciones=penalizaciones + 1
            if penalizaciones==3:
                return True
            else:
                relocate(penalizaciones)
                return False
...
def relocate(num):
    ...
    robot0Obj.position = [0.25, 0.0217, 0.55]
    robot0Obj.rotation = randomizeRotation([0, 1, 0, -0.5])
    robot0Obj.history.enqueue("Penalización nro: "+str(num+1))
    updateHistory()
...
robot0Obj.position = [0.25, 0.0217, 0.55]
##SE LO DEJO RECTO, PODRIA GIRARLE AL ARRANCAR CON LA DE ABAJO##
#robot0Obj.rotation = randomizeRotation([0, 1, 0, -0.5])
  ...
  #      robot1.restartController()
    ...
    if robot0Obj.inSimulation:
        ...
        if(robot0Obj.outOfDohyo()):
           ...
           # write_log(robot0Obj._name,"nada", "nada", "Salida dohyo", str(timeElapsed))
    ...
    if robot0Obj.inSimulation:
        ...
        if r0ts>timeOut:
            ...
            robot0Obj.inSimulation=False
            #robot1Obj.inSimulation=False
            #write_log(robot0Obj._name,"nada", "nada", "Timeout", str(timeElapsed))
            supervisor.wwiSendText("lostJ1")
    ...
    supervisor.wwiSendText(
        "update," + str(timeElapsed)+","+str(r0ts))

    # If the time is up
    if timeElapsed >= maxTime:
        finished = True
        #write_log(robot0Obj._name,"nada", "Empate", "Fin tiempo", str(timeElapsed))
        supervisor.wwiSendText("draw")

    if(robot0Obj.crashed()):
        finished = True
        #write_log(robot0Obj._name,"Nada", "Cancelado", "Crash", str(timeElapsed))
